        ...
        for(int i=0;i<simList.size();i++){
            ...
            if(lifecycleResult.get("status").equals("0")){
                ...
                if(!lifecycleArray.getJSONObject(0).get("lifecycle").equals(simList.get(i).getLifecycle())){
                    ...
                    cookieRequest = new Request.Builder()
                        ...
                        if(!onloneArray.getJSONObject(0).get("GPRSSTATUS").equals(simList.get(i).getState())){
                            if(onloneArray.getJSONObject(0).get("GPRSSTATUS").equals("")){
                                updateSimDate="update_"+simList.get(i).getId()+"_State_00";
                            }else{
                                updateSimDate="update_"+simList.get(i).getId()+"_State_"+onloneArray.getJSONObject(0).get("GPRSSTATUS");
                    ...
                    cookieRequest = new Request.Builder()
                            .url("https://api.iot.10086.cn/v2/userstatusrealsingle?appid=QHSLKYFFDM4TJO&transid=QHSLKYFFDM4TJO2019101410533080000001&ebid=0001000000009&token=3740d7d73ea1e228d08b5045434c09248e0883eb7d5ac0576e69e6fe2f6acd01&iccid="+simList.get(i).getSimNo())
                            .get()
                            .build();
                    execute = okHttpClient.newCall(cookieRequest).execute();
                    JSONObject cardResult = JSONObject.parseObject(execute.body().string());
                    if(cardResult.get("status").equals("0")){
                        JSONArray onloneArray = JSONObject.parseArray(cardResult.get("result").toString());
                        if(!onloneArray.getJSONObject(0).get("STATUS").equals(simList.get(i).getCardState())){

                            updateSimDate="update_"+simList.get(i).getId()+"_CardState_"+onloneArray.getJSONObject(0).get("CardState");
                            insertDate="insert_"+simList.get(i).getSimNo()+"_"+simList.get(i).getCardState()+"_"+onloneArray.getJSONObject(0).get("CardState")+"_3";
                                //记录 在线状态变更
                            rabbitTemplate.convertAndSend(insertDate);
                            rabbitTemplate.convertAndSend(updateSimDate);
                        }
